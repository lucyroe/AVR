...
def compare_variability_phase1_phase3(  # noqa: C901, PLR0912, PLR0915
    ...
    for phase in phases:
        ...
        stats_sorted = stats_sorted.drop(columns=0) # TODO: here is a mistake
        ...
        file_directory = directory / phase if phase == "phase1" else directory / "AVR" / "derivatives"
        stats_sorted.to_csv(file_directory / f"stats_{phase}.tsv")
    ...
    num_cols = len(variable_names[phases[1]]) * len(statistics)
        ...
        for variable in variable_names[phases[1]]:
            for statistic in statistics:
                ...
                if ax_counter >= first_subplot_last_row:
                    axes[ax_counter].set_xlabel(f"{variable} {statistic_names[statistic]}")
                ...
                table_normality = table_normality._append(
                    {
                        "Group": group,
                        "Variable": f"{statistic_names[statistic]} {variable}",
    ...
    for variable in variable_names[phases[1]]:
        for statistic in statistics:
            ...
            data_other_phase = data_all[(data_all["group"] == phases[1])][statistic + "_" + variable]
                ...
                for group2 in data_all["group"].unique():
                    if group1 != group2:
                        ...
                        results_table = results_table._append(
                            {
                                "Variable": f"{variable} {statistic_names[statistic]}",
                                "Group1": group1,
                                "Group2": group2,
